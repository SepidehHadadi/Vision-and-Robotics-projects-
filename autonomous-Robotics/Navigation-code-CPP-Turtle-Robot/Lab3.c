...
#define SIMULATION 64      // for robot_get_mode() function
...
void calibrate(int n){
  ...
  FILE *fp;
  int i,it;
  fp = fopen("measurment.txt","w+");
  if(fp == NULL) 
  {
    printf("Failed:%d ",-1);
  }
  /*fprintf(fp,"start of data writing");
  ...
  for (it=0; it<n; it++){
      for(i=0;i<NB_DIST_SENS;i++){
        ...
        fprintf(fp,"%d,",(int) wb_distance_sensor_get_value(ps[i]));
      }
      for(i=0;i<NB_LIGHT_SENS;i++){
        ls_offset[i] += (int) wb_light_sensor_get_value(ls[i]);
        //fprintf(fp,"%d,",ls_offset[i]);
        fprintf(fp,"%d,",(int) wb_light_sensor_get_value(ls[i]));
      }
  ...
  printf("Light sensor offset: %d,%d,%d,%d,%d,%d,%d,%d\n",ls_offset[0],ls_offset[1],ls_offset[2],ls_offset[3],ls_offset[4],ls_offset[5],ls_offset[6],ls_offset[7]);

  printf("Calibration is done.\n");
  printf("close file %d",-1);
  fclose(fp);
}
